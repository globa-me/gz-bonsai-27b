...
Каталог `0.3.0` использует Q1_0-файлы из официальных репозиториев PrismML. URL закреплены на конкретные Hugging Face commits, а LFS OID используется как ожидаемый SHA-256.
...
Версия `0.3.0` скачивает Q8 vision projector автоматически вместе с любой 27B-моделью и проверяет его размер и SHA-256. Модели 1.7B, 4B и 8B в текущем каталоге считаются text-only.
